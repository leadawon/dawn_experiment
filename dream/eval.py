...
class Dream(LM):
    def __init__(
        ...
        high_conf_threshold: Optional[float] = 0.9,
        ...
        fp_stats_path: Optional[str] = None,
    ...
    def _write_fp_stats(
        ...
        avg_forward_passes = (
            float(total_nfe) / float(num_generated_examples)
            if num_generated_examples > 0
            else 0.0
        )
        payload = {
            "timestamp": datetime.now().isoformat(),
            "total_examples": num_generated_examples,
            "returned_examples": num_returned_examples,
            "total_forward_passes": int(total_nfe),
            "avg_forward_passes": avg_forward_passes,
            "total_time_seconds": float(total_seconds),
            "tokens_generated": int(self.generated_token_num),
            "tokens_per_second": (
                float(self.generated_token_num) / float(total_seconds)
                if total_seconds > 0
                else 0.0
            ),
    ...
    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        ...
        num_generated_examples = max(len(res) - processed_count, 0)
        avg_nfe = (
            total_nfe / num_generated_examples
            if num_generated_examples > 0
            else 0.0
        )

        if self.show_speed:
            print(f"Time taken: {total_seconds} seconds")
            print(f"Generated token num: {self.generated_token_num}")
            print(f"Generated token num per second: {self.generated_token_num / total_seconds if total_seconds > 0 else 0.0}")
            print(f"Total NFE: {total_nfe}")
            print(f"Token per NFE: {self.generated_token_num / total_nfe if total_nfe > 0 else 0.0}")
                    ...
                    f.write(json.dumps(
                        {
                            ...
                            'Tokens per Second': self.generated_token_num / total_seconds if total_seconds > 0 else 0.0,
                            'Total NFE': total_nfe,
                            'Tokens per NFE': self.generated_token_num / total_nfe if total_nfe > 0 else 0.0,
                            'Average NFE': avg_nfe,
                        },
                        ensure_ascii=False
                    ) + "\n")
