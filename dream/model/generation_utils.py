...
class DreamGenerationMixin:
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        high_conf_threshold = kwargs.get("high_conf_threshold", 0.9)
    ...
    def _sample_block(
        ...
        for num_block in range(num_blocks):
            ...
            while True:
                ...
                if mask_index.sum() == 0:
                    break
                output, avg_attn_scores = self(x, attention_mask, tok_idx, return_attn_scores= (alg == 'dawn'))
                logits = output.logits
                logits = torch.cat([logits[:,:1], logits[:, :-1]], dim=1)
                ...
                mask_logits = logits[mask_index]
                nfe += 1
                if not alg == 'confidence_threshold' and not alg == 'dawn' and not alg == 'klass' and not alg == 'local_leap' and not alg == 'factor':
                    t = timesteps[i]
                    s = timesteps[i + 1]
                ...
                if alg == 'origin':
                    ...
                    adj_ti_mask = adj_ti_mask.any(dim=1)
                    node_mask = mask_index & (x0_p >= conf_threshold) & (x0_p < high_conf_threshold) & ~transfer_index_a & ~adj_ti_mask
